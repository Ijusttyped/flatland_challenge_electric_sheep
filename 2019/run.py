...
# of the agents
#####################################################################
def my_controller(obs, number_of_agents, agent):
    _action = {}
    for _idx in range(number_of_agents):
        if obs[_idx] is not None:
            norm_obs = normalize_observation(obs[_idx], tree_depth=2)
            _action[_idx] = agent.act(norm_obs, 0)
    return _action
...
# Load the agent
agent = DQNAgent(state_size=231, action_size=5)
...
while True:
    ...
    while True:
        ...
        # defined controller
        time_start = time.time()
        action = my_controller(observation, number_of_agents, agent)
